...
try:
    ...
    for i in range(N):
        ...
        if NORM=='inf':
            ...
            for j in range(col+1):
                ...
                m.addConstr(abs_norm_value1[j]==norm_vector_p_plus[j],"infNorm_abs_norm_value_"+str(j)+"_i="+str(i))

                # abs_norm_value2[j]=abs(abs_norm_value1[j])
                m.addGenConstrAbs(abs_norm_value2[j],abs_norm_value1[j],"infNorm_abs_value_"+str(j)+"_i="+str(i))

            m.addConstr(gp.quicksum(abs_norm_value2[j] for j in range(col+1))<=lambda_var,"Lambda_norm_constraint"+"_i="+str(i))
            ...
            for j in range(col+1):
                ...
                m.addConstr(abs_norm_value1[j]==norm_vector_p_plus[j],"1Norm_abs_norm_value_"+str(j)+"_i="+str(i))
                m.addGenConstrAbs(abs_norm_value2[j],abs_norm_value1[j],"1Norm_abs_value_"+str(j)+"_i="+str(i))

            m.addGenConstrMax(max_norm_component,abs_norm_value2)#,"Max_norm_constraint_i="+str(i))
            m.addConstr(max_norm_component<=lambda_var,"Lambda_norm_constraint_i="+str(i))
    ...
    plot_name="Plots/Support_vector_reg_v3_plot_NORM_{}_wassradi_{}_epsil_{}.png".format(NORM,e_wradi,epsilon)
