...
def plot_data(x,y,w,plot_name):
    ...
    # x_predict=[0,1,2,3,4]
    # for xi in x_predict:
    #     y_predict.append(fit_obj.predict(xi)[0])
...
try:
    ...
    for i in range(N):
        ...
        if NORM=='inf':
            ...
            for j in range(col):
                ...
                m.addConstr(abs_norm_value1_helper[j]==w[j],"1Norm_abs_norm_value_"+str(j)+"_"+str(i))
                m.addGenConstrAbs(abs_norm_value1[j],abs_norm_value1_helper[j],"1Norm_abs_value_"+str(j)+"_"+str(i))
    ...
    plot_name="Plots/DRSVM_v1.2_plot_NORM_{}_wassradi_{}_kappa_{}.png".format(NORM,e_wradi,kappa)
    plot_data(x,y,w_sol,plot_name)

    # print(m.display())
    m.write('DRSVM_v1.2.lp')
