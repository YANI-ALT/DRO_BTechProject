...
def Cross_Validation_5fold(data,params,test_data=None):
    X=data['X']
    y=data['y']
    SPLITS=5
    ...
    if test_data!=None:
        split_scores["Test-Data"]={}
        for key_ in w_values:
            y_pred_test=Predict(w_values[key_],{"X":test_data['X'],"y":test_data['y']})
            score=roc_auc_score(test_data['y'],y_pred_test)
            split_scores["Test-Data"][key_]=score
...
def run_cv(Norms,wasserstein_radi,kappa,datasetName):
    ...
    # split the data and perform 5cv on the train
    ...
    for norm in Norms:
        for w_radi in wasserstein_radi:
            for k in kappa:
                ...
                results,split_data=Cross_Validation_5fold(data,params,test_data)
            ...
            # plot_5CV_data(split_data,data,params,plot_name)
            # print(results)
    print(df_main.drop(["Split_5","Split_1","Split_2","Split_3","Split_4"],axis=1))
    print("FILE SAVED : Pickle_data/5CV_{}_NORM_{}_wassradi_{}_kappa_{}.csv".format(datasetName,Norms,wasserstein_radi,kappa))
    df_main.to_csv("Pickle_data/5CV_{}_NORM_{}_wassradi_{}_kappa_{}.csv".format(datasetName,Norms,wasserstein_radi,kappa))

#########################################


run_cv(["two"],[0.1],[0.5],"MNIST")
